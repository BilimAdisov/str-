...
int s21_sprintf(char* str, const char* format, ...) {
  ...
  while (*format_copy) {
    if (*format_copy == '%') {
      ...
      str =
          ParseSpecifiers(str, src, &format_copy, &settings, &arg, specifiers);
    } else {
      *str = *format_copy;
      str++;
      format_copy++;
...
void GetPrecision(char** format, Settings* settings, va_list* arg) {
  (*format)++;
  format =
      FormatShift(format, &settings->precision, arg, &settings->round_float);
}
...
char* SetFloat(char* str, va_list* arg, Settings* settings) {
  ...
  if ((round(fract * pow(10, settings->precision)) /
       pow(10, settings->precision)) == 1) {
    decimal += 1;
  }
  int size = 0;
  int decimal_size = 0;
...
s21_size_t SetSize(Settings* settings, long int decimal) {
  ...
  if (size == 0 && !settings->precision && !settings->width && !settings->space)
    size++;
